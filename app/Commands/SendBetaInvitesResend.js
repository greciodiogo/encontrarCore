...
const ResendEmailService = require('../Helpers/sendEmailResend')

class SendBetaInvitesResend extends Command {
  static get signature() {
    return 'send:beta-invites-resend'
  }

  static get description() {
    return 'Enviar convites de beta test usando Resend (3000 emails/mês grátis)'
  }

  async handle(args, options) {
    this.info('🚀 Iniciando envio de convites via Resend...\n')
    ...
    for (let i = 0; i < betaTesters.length; i++) {
      ...
      try {
        ...
        const result = await emailService.send({
          ...
          tags: {
            category: 'beta-test',
            campaign: 'beta-invites-2025'
          }
        })
        
        if (result.success) {
          this.success(`   ✅ Enviado! ID: ${result.messageId}`)
          successCount++
        ...
        // Delay de 1 segundo entre emails (Resend é mais rápido)
    ...
    this.info('\n✨ Processo concluído!')
    this.info(`\n💡 Acesse https://resend.com/emails para ver os detalhes dos envios`)
  }
}

module.exports = SendBetaInvitesResend
