...
class PaginationHelper {
   ...
   * @param {number} perPage The number of result per page. Optionally if `custom_build === false`
   ...
   * @returns Object
   */
  constructor(result, request, { custom_build }, perPage, page, total) {
    this.total = Number(result.total) || total;
    this.perPage = Number(result.perPage) || Number(perPage);
    this.page = Number(result.page) || Number(page);
    this.lastPage = Number(result.lastPage);
  ...
  /**
   * Get the "perPage" property
   * @method getPerPage
   * @returns {number} The current number of results per query.
   */
  getPerPage() {
    return this.perPage ? this.perPage : null;
  }
   ...
   *
   * @method paginate
   * @returns {{total: number, perPage: number, page: number, lastPage: number, next_page_url: string, prev_page_url: string, from: number, to: number, data: Array.<object>}}
   *
   */
  get paginate() {
    return {
